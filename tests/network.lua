...
return {
	...
	['Functions'] = {
		...
		['create event'] = function(profiler,plr)
			...
			assert(typeof(result) == 'Instance' and result:IsA('RemoteEvent'),"Failed to create RemoteEvent")
		end;
		
		['create bindable'] = function(profiler,plr)
			local result = Network.CreateBindableEvent('BENCHMARK_CREATE')
			assert(typeof(result) == 'Instance' and result:IsA('BindableEvent'),"Failed to create BindableEvent")
		end;
		
		['hook event'] = function(profiler,plr)
			local result; result = Network:HookEvent('BENCHMARK_EVENT',function()
				Network:UnhookEvent('BENCHMARK_EVENT')
			end)
			assert(typeof(result) == 'Instance' and result:IsA('RemoteEvent'),"Failed to hook RemoteEvent")
		end;
		...
		['bind event'] = function(profiler,plr)
			...
			assert(typeof(result) == 'Instance' and result:IsA('BindableEvent'),"Failed to hook BindableEvent")
		end;
		
		['pipeline'] = function(profiler,plr)
			local result; result = Network:BindEvent('BENCHMARK_BINDABLE',function()
				result = nil
			end)
			assert(typeof(result) == 'Instance' and result:IsA('BindableEvent'),"Failed to connect event")
